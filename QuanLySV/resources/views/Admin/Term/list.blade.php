            ...
            <div class="row page-titles mx-0">
                ...
                <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><a>Term List</a></li>
                    </ol>
                </div>
            </div>
                    ...
                    <div class="card">
                        <div class="card-header card-margin">
                            <div>
                                ...
                                <a href="{{ route('admin.addTerm.form') }}"><Span class="btn btn-primary">
                                        <i class="fa-solid fa-user-plus"></i> Add</Span></a>

                                <a style="display: none" id="delete-term-btn"><Span class="btn btn-primary">
                                        ...
                                        <tbody>
                                            @foreach ($data as $item)
                                                @php
                                                    $i++;
                                                @endphp
                                                <tr id="termValue-{{ $item->term_id }}"
                                                    class="green-hover
                                                    @if ($i % 2 == 0) tr-background @endif ">
                                                    <td><input class="term-checked" type="checkbox" name="termChecked[]"
                                                            value="{{ $item->term_id }}">
                                                    </td>
                                                    <td>{{ $item->name }}</td>
                                                    <td>
                                                        <input type="checkbox" name="checked" class="checkOpenSemester"
                                                            data-open-semester="{{ $item->term }}"
                                                            {{ $item->on_off != 0 ? 'checked' : '' }} disabled>
                                                    </td>

                                                    <td><span><a href="{{ route('admin.updateTerm.form', ['term_id' => $item->term_id]) }}"
